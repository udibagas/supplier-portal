<template>
    <div>
        <el-form :inline="true" style="text-align:right" @submit.native.prevent="() => { return }">
            <el-form-item>
                <el-button icon="el-icon-plus" @click="openForm({})" type="primary">ADD NEW DOCUMENT TYPE</el-button>
            </el-form-item>
        ...
        <el-dialog :visible.sync="showForm" :title="!!formModel.id ? 'EDIT DOCUMENT TYPE' : 'ADD NEW DOCUMENT TYPE'" width="500px" v-loading="loading" :close-on-click-modal="false">
...
export default {
    ...
    methods: {
        ...
        store() {
            this.loading = true;
            axios.post('/vendorDocumentType', this.formModel).then(r => {
                this.showForm = false;
                this.$message({
        ...
        update() {
            this.loading = true;
            axios.put('/vendorDocumentType/' + this.formModel.id, this.formModel).then(r => {
                this.showForm = false
                this.$message({
                    message: 'Data berhasil disimpan.',
                    type: 'success',
        ...
        deleteData(id) {
            this.$confirm('Anda yakin akan menghapus data ini?', 'Warning', { type: 'warning' }).then(() => {
                axios.delete('/vendorDocumentType/' + id).then(r => {
                    this.requestData();
                    this.$message({
        ...
        requestData() {
            ...
            this.loading = true;
            axios.get('/vendorDocumentType', {params: params}).then(r => {
