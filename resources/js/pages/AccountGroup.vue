<template>
    <div>
        <el-form :inline="true" style="text-align:right" @submit.native.prevent="() => { return }">
            <el-form-item>
                <el-button icon="el-icon-plus" @click="openForm({role: 0, password: ''})" type="primary">ADD NEW ACCOUNT GROUP</el-button>
            </el-form-item>
        ...
        <el-dialog :visible.sync="showForm" :title="!!formModel.id ? 'EDIT ACCOUNT GROUP' : 'ADD NEW ACCOUNT GROUP'" width="500px" v-loading="loading" :close-on-click-modal="false">
...
export default {
    ...
    methods: {
        ...
        store() {
            this.loading = true;
            axios.post('/accountGroup', this.formModel).then(r => {
                this.showForm = false;
                this.$message({
        ...
        update() {
            this.loading = true;
            axios.put('/accountGroup/' + this.formModel.id, this.formModel).then(r => {
                this.showForm = false
                this.$message({
                    message: 'Data berhasil disimpan.',
                    type: 'success',
        ...
        deleteData(id) {
            this.$confirm('Anda yakin akan menghapus data ini?', 'Warning', { type: 'warning' }).then(() => {
                axios.delete('/accountGroup/' + id).then(r => {
                    this.requestData();
                    this.$message({
        ...
        requestData() {
            ...
            this.loading = true;
            axios.get('/accountGroup', {params: params}).then(r => {
