<template>
  <div>
    <el-page-header @back="$emit('goBack')" content="QUOTATION REQUEST"></el-page-header>
    ...
    >
      ...
      >
        <template slot-scope="scope">
          ...
          >{{scope.row.status ? 'Submitted' : 'Draft'}}</el-tag>
        </template>
      </el-table-column>
        ...
        <template slot-scope="scope">
          <el-dropdown>
            ...
            <el-dropdown-menu slot="dropdown">
              ...
              >
                <i class="el-icon-zoom-in"></i> Show Detail
              </el-dropdown-item>
              <!-- MENU UNTUK PROCUREMENT -->
              <el-dropdown-item
                v-if="$store.state.user.role == 25"
                @click.native.prevent="reviewQuotationRequest(scope.row)"
              >
                <i class="el-icon-chat-line-square"></i> Review Request
              </el-dropdown-item>
              <el-dropdown-item
                v-if="$store.state.user.role == 25"
                @click.native.prevent="openAssignmentForm(scope.row)"
              >
                <i class="el-icon-share"></i> Assign to Vendor
              </el-dropdown-item>
              <el-dropdown-item
                v-if="$store.state.user.role == 25"
                @click.native.prevent="showQuotation = true"
              >
                <i class="el-icon-document-copy"></i> Show Quotation
              </el-dropdown-item>
              <el-dropdown-item
                v-if="$store.state.user.role == 31"
                @click.native.prevent="createQuotation(scope.row)"
              >
                <i class="el-icon-plus"></i> Create Quotation
              </el-dropdown-item>
              <!-- CUMA USER YG CREATE YG BISA UBAH/HAPUS -->
              <el-dropdown-item
                v-if="$store.state.user.id == scope.row.user_id"
    ...
    <el-dialog
      ...
      top="60px"
      width="90%"
      title="DETAIL QUOTATION REQUEST"
    ...
    <el-dialog
      ...
      :visible.sync="showAssignmentForm"
    >
      <el-form label-width="150px" label-position="left">
        ...
        <el-form-item label="Assign To Vendor" :class="formErrors.vendors ? 'is-error' : ''">
          <el-select
            ...
            v-model="formModelAssignment.vendors"
            placeholder="Vendor"
            style="width:100%"
          >
            <el-option
              v-for="vendor in $store.state.vendorList"
              :value="vendor.id"
              :label="vendor.name"
              :key="vendor.id"
            ></el-option>
    ...
    <el-dialog
      ...
      :title="!!formModel.id ? 'EDIT QUOTATION REQUEST' : 'ADD NEW QUOTATION REQUEST'"
      v-loading="loading"
    ...
    >
        ...
        <el-row :gutter="50">
          ...
          <el-col :span="12">
            ...
            <el-form-item label="Department" :class="formErrors.department_id ? 'is-error' : ''">
              ...
              >
                <el-option
                  v-for="(t, i) in $store.state.departmentList"
                  :value="t.id"
                  :label="t.name"
                  :key="i"
                ></el-option>
              ...
              >{{formErrors.department_id[0]}}</div>
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
        ...
        <el-table-column prop="requested_delivery_date" label="Delivery Date" min-width="110px">
          <template slot-scope="scope">
            <el-date-picker
              ...
              format="dd-MMM-yyyy"
              type="date"
              v-model="scope.row.requested_delivery_date"
            ></el-date-picker>
          </template>
        </el-table-column>
      ...
      <span slot="footer" class="dialog-footer">
        <el-button plain type="primary" @click="() => !!formModel.id ? update(0) : store(0)">
          <i class="el-icon-success"></i> SAVE AS DRAFT
        </el-button>
        <el-button type="primary" @click="() => !!formModel.id ? update(1) : store(1)">
          <i class="el-icon-success"></i> SAVE AND SUBMIT
        </el-button>
        <el-button type="info" @click="showForm = false">
          <i class="el-icon-error"></i> CANCEL
        </el-button>
      </span>
    </el-dialog>
      ...
      <el-table :data="quotations" height="calc(100vh - 420px)" stripe>
        ...
        <el-table-column v-for="i in ['A', 'B']" :key="i" header-align="center">
          <el-dropdown slot="header">
            <span class="el-dropdown-link">Vendor {{i}}</span>
            <el-dropdown-menu slot="dropdown">
          ...
          <el-table-column label="Price" min-width="120px" align="right" header-align="center">
            <template>Rp. 1.000.000</template>
          </el-table-column>
          <el-table-column
            prop="requested_delivery_date"
            label="Delivery Date"
    ...
    <el-dialog title="QUOTATION FORM" center top="60px" :visible.sync="quotationForm" width="95%">
        ...
        <el-row :gutter="30">
          <el-col :span="12">
            ...
            <el-form-item
              ...
              :class="formErrors.quotation_date ? 'is-error' : ''"
            >
              <el-date-picker
                v-model="formModel.quotation_date"
                style="width:100%"
                value-format="yyyy-MM-dd"
                format="dd-MMM-yyyy"
              ...
              <div
                class="el-form-item__error"
                v-if="formErrors.quotation_date"
              >{{formErrors.quotation_date[0]}}</div>
            </el-form-item>
            <el-form-item label="Quotation Number">
              <el-input v-model="formModel.quotation_number" placeholder="Quotation Number"></el-input>
              <div
                class="el-form-item__error"
                v-if="formErrors.quotation_number"
              >{{formErrors.quotation_number[0]}}</div>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            ...
            <el-form-item
              ...
              :class="formErrors.term_of_payment ? 'is-error' : ''"
            >
              <el-select
                style="width:100%"
                ...
                filterable
                default-first-option
                v-model="formModel.term_of_payment"
              >
                <el-option
                  v-for="(top, i) in $store.state.paymentTermList"
                  :key="i"
              ...
              <div
                class="el-form-item__error"
                v-if="formErrors.term_of_payment"
              >{{formErrors.term_of_payment[0]}}</div>
            </el-form-item>
      ...
      <el-table :data="formModel.items">
        ...
        <el-table-column header-align="center" label="Quotation">
          <el-table-column label="Qty" width="100px" header-align="center">
            <template slot-scope="scope">
              <el-input
                v-model="scope.row.promised_qty"
                size="small"
                placeholder="Qty"
                type="number"
              ></el-input>
          ...
          <el-table-column label="Delivery Date" width="170px" header-align="center">
            <template slot-scope="scope">
              <el-date-picker
                v-model="scope.row.promised_delivery_date"
                size="small"
                style="width:100%"
                value-format="yyyy-MM-dd"
                format="dd-MMM-yyyy"
      ...
      <div slot="footer">
        <el-button
          ...
          @click="storeQuotation(0)"
        >SAVE AS DRAFT</el-button>
        <el-button type="primary" icon="el-icon-success" @click="storeQuotation(1)">SUBMIT</el-button>
...
export default {
      ...
      quotations: [
        {
          ...
          part_number: "6565465",
          part_description: "Desktop HP",
          requested_qty: 1,
          requested_delivery_date: "2019-08-29",
          remark: "-",
          attachment: null,
          created_at: "2019-08-19 16:06:11",
          updated_at: "2019-08-19 16:06:11"
        },
        {
          id: 15,
          quotation_request_id: 2,
          part_number: "wd",
          part_description: "efwwfw",
          requested_qty: 100,
          requested_delivery_date: "2019-08-30",
          remark: "-",
          attachment: null,
          created_at: "2019-08-19 16:06:11",
          updated_at: "2019-08-19 16:06:11"
  ...
  methods: {
    ...
    assignQuotation() {
      axios
        .post("/quotationRequestAssignment", this.formModelAssignment)
        .then(r => {
    ...
    createQuotation(data) {
      ...
      this.formModel.vendor_name = data.vendor_assignments.find(
        a => a.vendor_id == this.$store.state.user.vendor_id
      ).vendor.name;
      this.formModel.quotation_request_id = data.id;
    ...
    updateQuotation(status) {
      this.loading = true;
      axios
        .put("/quotation/" + this.formModel.id, this.formModel)
        .then(r => {
          this.$message({
            message: "Data telah disimpan",
            type: "success",
    ...
    deleteItem(index, item_id) {
      if (!!item_id) {
        axios
          .delete("/quotationRequestItem/" + item_id)
          .then(r => {
    ...
    store(status) {
      ...
      this.formModel.status = status;
      axios
        .post("/quotationRequest", this.formModel)
        .then(r => {
          this.showForm = false;
          this.$message({
    ...
    update(status) {
      ...
      this.formModel.status = status;
      axios
        .put("/quotationRequest/" + this.formModel.id, this.formModel)
        .then(r => {
          this.showForm = false;
          this.$message({
            message: "Data berhasil disimpan.",
    ...
    requestData() {
      ...
      this.loading = true;
      axios
        .get("/quotationRequest", { params: params })
        .then(r => {
